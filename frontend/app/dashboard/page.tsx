...
const CREATOR_MODE_META: Record<string, { label: string; emoji: string; desc: string; color: string; border: string; bg: string }> = {
...
export default function DashboardPage() {
  ...
  const { user, isAuthenticated, hydrated, saveProfile } = useAuth()
  ...
  useEffect(() => {
    const close = () => { setShowDomainPicker(false); setShowAudiencePicker(false); setShowModePicker(false) }
          ...
          <div>
            <div className="flex items-center gap-2 mb-3 flex-wrap">
              ...
              <div className="relative">
                <button
                  onClick={(e) => { e.stopPropagation(); setShowDomainPicker(v => !v); setShowAudiencePicker(false) }}
                ...
                >
                  <span className="text-[10px] font-mono font-semibold text-cyan-400 uppercase tracking-widest">
                    {savingProfile ? '…' : (domainLabel || '＋ Set Domain')}
                  </span>
                  <span className="text-cyan-400/60 text-[10px]">▾</span>
                </button>
                {showDomainPicker && (
                  <div onClick={(e) => e.stopPropagation()} className="absolute left-0 top-full mt-2 z-50 bg-[#0d1117] border border-white/[0.12] rounded-2xl shadow-2xl shadow-black/40 p-1.5 min-w-[200px]">
              ...
              <div className="relative">
                <button
                  onClick={(e) => { e.stopPropagation(); setShowAudiencePicker(v => !v); setShowDomainPicker(false) }}
                ...
                >
                  <span className="text-[10px] font-mono font-semibold text-violet-400 uppercase tracking-widest">
                    {savingProfile ? '…' : (audienceLabel || '＋ Set Audience')}
                  </span>
                  <span className="text-violet-400/60 text-[10px]">▾</span>
                </button>
                {showAudiencePicker && (
                  <div onClick={(e) => e.stopPropagation()} className="absolute left-0 top-full mt-2 z-50 bg-[#0d1117] border border-white/[0.12] rounded-2xl shadow-2xl shadow-black/40 p-1.5 min-w-[190px]">
              ...
              <div className="relative">
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full border transition-all cursor-pointer hover:opacity-90 ${
                    user?.creatorMode
                      ? `${CREATOR_MODE_META[user.creatorMode]?.bg || 'bg-white/[0.06]'} ${CREATOR_MODE_META[user.creatorMode]?.border || 'border-white/[0.15]'}`
                      : 'bg-white/[0.04] border-white/[0.12] hover:border-white/[0.20]'
                  }`}
                >
                  <span className={`text-[10px] font-mono font-semibold uppercase tracking-widest ${CREATOR_MODE_META[user?.creatorMode || '']?.color || 'text-white/50'}`}>
                    {savingProfile ? '…' : user?.creatorMode
                      ? `${CREATOR_MODE_META[user.creatorMode]?.emoji} ${CREATOR_MODE_META[user.creatorMode]?.label}`
                      : '＋ Set Mode'}
                  </span>
                  <span className={`text-[10px] opacity-60 ${CREATOR_MODE_META[user?.creatorMode || '']?.color || 'text-white/50'}`}>▾</span>
                </button>
                {showModePicker && (
                  <div onClick={(e) => e.stopPropagation()} className="absolute left-0 top-full mt-2 z-50 bg-[#0d1117] border border-white/[0.12] rounded-2xl shadow-2xl shadow-black/40 p-1.5 min-w-[220px]">
